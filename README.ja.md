...
```matlab
dataset_name  = 'samples';  % レポートに表示される名前。自由に指定できる
bvec_file     = 'samples.bvec';
bval_file     = 'samples.bval';
...
html_name     = 'samples'; % フォルダ名になる。複数回実行する場合は上書き注意
```
...
（上記の場合は `samples`というフォルダ）  
フォルダ内の `samples.html`をダブルクリックするとレポートがブラウザに表示される
